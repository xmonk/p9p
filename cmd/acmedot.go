...
func main() {
	...
	if !bytes.Equal(body, f.body) {
		log.Fatalf("file contents mismatch")
	}
	fmt.Printf("%s:#%d,#%d\n", f.name, f.offset0, f.offset1)
}
...
func acmeCurrentFile() (*acmeFile, error) {
	...
	w := &acmeFile{
		name:       tag[0:i],
		body:       body,
		offset0:    runeOffset2ByteOffset(body, q0),
		offset1:    runeOffset2ByteOffset(body, q1),
		runeOffset: q0,
	}
	return w, nil
}
